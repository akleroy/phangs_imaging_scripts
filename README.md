...
the CASA shell. Note that if you are running within monolithic CASA and want
to make use of the single dish imaging capabilities, you will need to run
a pipeline version.

Note that by default, this will not install CASA-related packages, and so will 
...
```bash
pip install phangsPipeline[casa] @ git+https://github.com/akleroy/phangs_imaging_scripts.git
```
